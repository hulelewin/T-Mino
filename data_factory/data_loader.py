...
class CreditSegLoader(object):
    def __init__(self, data_path, win_size, step, mode="train"):
        ...
        self.win_size = win_size
        self.scaler = StandardScaler()
        data = np.load(data_path + "/Credit_train.npy")
        self.scaler.fit(data)
        data = self.scaler.transform(data)
        test_data = np.load(data_path + "/Credit_test.npy")
        self.test = self.scaler.transform(test_data)
        self.train = data
        data_len = len(self.train)
        self.val = self.train[(int)(data_len * 0.8):]
        ...
        self.test_labels = pd.read_csv(data_path + '/Credit_test_label.csv').values[:, :]
...
class MSLSegLoader(object):
    def __init__(self, data_path, win_size, step, mode="train"):
        ...
        self.test_labels = pd.read_csv(data_path + '/MSL_test_label.csv').values[:, :]
    ...
    def __getitem__(self, index):
        ...
        if self.mode == "train":
            ...
            a = np.float32(self.test_labels[index:index + self.win_size])
            return np.float32(self.test[index:index + self.win_size]), np.float32(
                self.test_labels[index:index + self.win_size])
        else:
            return np.float32(self.test[
...
class SMAPSegLoader(object):
    def __init__(self, data_path, win_size, step, mode="train"):
        ...
        self.test_labels = pd.read_csv(data_path + '/SMAP_test_label.csv').values[:, :]
...
class SMDSegLoader(object): 
    def __init__(self, data_path, win_size, step, mode="train"):
        ...
        self.test_labels = pd.read_csv(data_path + '/SMD_test_label.csv').values[:, :]
...
class NIPS_TS_WaterSegLoader(object):
    def __init__(self, data_path, win_size, step, mode="train"):
        ...
        self.test_labels = pd.read_csv(data_path + '/NIPS_TS_Water_test_label.csv').values[:, :]
...
class NIPS_TS_SwanSegLoader(object):
    def __init__(self, data_path, win_size, step, mode="train"):
        ...
        self.test_labels = pd.read_csv(data_path + '/NIPS_TS_Swan_test_label.csv').values[:, :]
...
class SWATSegLoader(Dataset):
    def __init__(self, root_path, win_size, step=1, flag="train"):
        ...
        test_labels = pd.read_csv(os.path.join(root_path, "swat_test_label.csv"))
        labels = test_labels
...
def get_loader_segment(index, data_path, batch_size, win_size=100, step=100, mode='train',
                       dataset='KDD'):  
    if (dataset == 'SMD'):
        ...
        dataset = CreditSegLoader(data_path, win_size, 1, mode)
    shuffle = False
    if mode == 'train':
        shuffle = True
